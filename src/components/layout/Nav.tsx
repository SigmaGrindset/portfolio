          ...
          <ul className="flex gap-8">
            {links.map((l) => (
              <li key={l.href}>
                <a
                  href={l.href}
                  className="text-sm font-medium text-[color:var(--color-text-secondary)] hover:text-[color:var(--color-text)] transition-colors"
                >
                  <span className="font-mono text-xs text-[color:var(--color-accent)] mr-1.5">{l.num}</span>
                  {l.label}
                </a>
              </li>
            ))}
